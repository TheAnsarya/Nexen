-- Blargg test protocol handler for Nexen --testRunner.
-- Polls address $6000 in NES CPU memory for blargg result codes.
...
local function onFrameEnd()
	...
	if consoleType ~= "Nes" then
		emu.log("blargg-test: wrong console type: " .. tostring(consoleType))
		emu.stop(90)
		return
	end
	...
	if (frameCount % POLL_INTERVAL) == 0 and frameCount > 0 then
		local result = emu.read(RESULT_ADDR, emu.memType.nesMemory)

		if result ~= 0x80 then
			if result == 0x00 then
				emu.log("blargg-test: PASS at frame " .. tostring(frameCount))
				emu.stop(0)
			else
				emu.log("blargg-test: FAIL (test #" .. tostring(result) .. ") at frame " .. tostring(frameCount))
				emu.stop(1)
			end
			return
		end
	...
	if frameCount >= MAX_FRAMES then
		emu.log("blargg-test: TIMEOUT at frame " .. tostring(frameCount))
		emu.stop(2)
	end
end

emu.addEventCallback(onFrameEnd, emu.eventType.endFrame)
emu.log("blargg-test: started, polling $6000 every " .. tostring(POLL_INTERVAL) .. " frames, max " .. tostring(MAX_FRAMES))
