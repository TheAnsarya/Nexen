...
namespace {
	vector<GenesisCompatibilityRomCase> BuildGenesisCompatibilityCorpus() {
		...
		corpus.push_back({"golden-axe.bin", vector<uint8_t>(0x8000, 0x4E)});
		corpus.push_back({"streets-of-rage.bin", vector<uint8_t>(0x8000, 0x4E)});
		return corpus;
	}
	...
	TEST(GenesisCompatibilityHarnessTests, MapperEdgeAndOwnershipCheckpointsArePresentAndPassing) {
		...
		for (const GenesisCompatibilityEntry& entry : result.Entries) {
			...
			EXPECT_TRUE(hasPassingCheckpoint("GEN-COMPAT-BOOT-TO-PLAY-READINESS"));
			...
			EXPECT_TRUE(hasPassingCheckpoint("GEN-COMPAT-FIRST20S-SEQUENCING-STRESS"));
	...
	TEST(GenesisCompatibilityHarnessTests, BootToPlayReadinessCheckpointContextIsDeterministicAcrossRuns) {
		GenesisM68kBoundaryScaffold scaffold;
		vector<GenesisCompatibilityRomCase> corpus = BuildGenesisCompatibilityCorpus();

		auto readBootToPlayReadinessContext = [&](const GenesisCompatibilityMatrixResult& result, const string& romName) {
			auto entryIt = std::find_if(result.Entries.begin(), result.Entries.end(), [&](const GenesisCompatibilityEntry& entry) {
				return entry.Name == romName;
			});
			if (entryIt == result.Entries.end()) {
			...
			auto checkpointIt = std::find_if(entryIt->Checkpoints.begin(), entryIt->Checkpoints.end(), [](const GenesisCompatibilityCheckpoint& checkpoint) {
				return checkpoint.Id == "GEN-COMPAT-BOOT-TO-PLAY-READINESS";
			});
			if (checkpointIt == entryIt->Checkpoints.end()) {
				ADD_FAILURE() << "Missing GEN-COMPAT-BOOT-TO-PLAY-READINESS checkpoint for " << romName;
				return string();
			}
		...
		for (const GenesisCompatibilityRomCase& romCase : corpus) {
			string contextA = readBootToPlayReadinessContext(runA, romCase.Name);
			string contextB = readBootToPlayReadinessContext(runB, romCase.Name);
			EXPECT_EQ(contextA, contextB);
		}
	}

	TEST(GenesisCompatibilityHarnessTests, RealRomStartupSmokeCheckpointContextIsDeterministicAcrossRuns) {
		GenesisM68kBoundaryScaffold scaffold;
		vector<GenesisCompatibilityRomCase> corpus = BuildGenesisCompatibilityCorpus();

		auto readRealRomStartupSmokeContext = [&](const GenesisCompatibilityMatrixResult& result, const string& romName) {
			auto entryIt = std::find_if(result.Entries.begin(), result.Entries.end(), [&](const GenesisCompatibilityEntry& entry) {
				return entry.Name == romName;
			});
			if (entryIt == result.Entries.end()) {
			...
			auto checkpointIt = std::find_if(entryIt->Checkpoints.begin(), entryIt->Checkpoints.end(), [](const GenesisCompatibilityCheckpoint& checkpoint) {
				return checkpoint.Id == "GEN-COMPAT-REALROM-STARTUP-SMOKE";
			});
			if (checkpointIt == entryIt->Checkpoints.end()) {
				ADD_FAILURE() << "Missing GEN-COMPAT-REALROM-STARTUP-SMOKE checkpoint for " << romName;
				return string();
			}
		...
		for (const GenesisCompatibilityRomCase& romCase : corpus) {
			string contextA = readRealRomStartupSmokeContext(runA, romCase.Name);
			string contextB = readRealRomStartupSmokeContext(runB, romCase.Name);
			EXPECT_EQ(contextA, contextB);
		}
	}

	TEST(GenesisCompatibilityHarnessTests, StartupInputWindowCheckpointContextIsDeterministicAcrossRuns) {
		GenesisM68kBoundaryScaffold scaffold;
		vector<GenesisCompatibilityRomCase> corpus = BuildGenesisCompatibilityCorpus();

		auto readStartupInputWindowContext = [&](const GenesisCompatibilityMatrixResult& result, const string& romName) {
			auto entryIt = std::find_if(result.Entries.begin(), result.Entries.end(), [&](const GenesisCompatibilityEntry& entry) {
				return entry.Name == romName;
			});
			if (entryIt == result.Entries.end()) {
			...
			auto checkpointIt = std::find_if(entryIt->Checkpoints.begin(), entryIt->Checkpoints.end(), [](const GenesisCompatibilityCheckpoint& checkpoint) {
				return checkpoint.Id == "GEN-COMPAT-STARTUP-INPUT-WINDOW";
			});
			if (checkpointIt == entryIt->Checkpoints.end()) {
				ADD_FAILURE() << "Missing GEN-COMPAT-STARTUP-INPUT-WINDOW checkpoint for " << romName;
				return string();
			}
		...
		for (const GenesisCompatibilityRomCase& romCase : corpus) {
			string contextA = readStartupInputWindowContext(runA, romCase.Name);
			string contextB = readStartupInputWindowContext(runB, romCase.Name);
			EXPECT_EQ(contextA, contextB);
		}
	}

	TEST(GenesisCompatibilityHarnessTests, StartupMemMapParityCheckpointContextIsDeterministicAcrossRuns) {
		GenesisM68kBoundaryScaffold scaffold;
		vector<GenesisCompatibilityRomCase> corpus = BuildGenesisCompatibilityCorpus();

		auto readStartupMemMapParityContext = [&](const GenesisCompatibilityMatrixResult& result, const string& romName) {
			auto entryIt = std::find_if(result.Entries.begin(), result.Entries.end(), [&](const GenesisCompatibilityEntry& entry) {
				return entry.Name == romName;
			});
			if (entryIt == result.Entries.end()) {
			...
			auto checkpointIt = std::find_if(entryIt->Checkpoints.begin(), entryIt->Checkpoints.end(), [](const GenesisCompatibilityCheckpoint& checkpoint) {
				return checkpoint.Id == "GEN-COMPAT-STARTUP-MEMMAP-PARITY";
			});
			if (checkpointIt == entryIt->Checkpoints.end()) {
				ADD_FAILURE() << "Missing GEN-COMPAT-STARTUP-MEMMAP-PARITY checkpoint for " << romName;
				return string();
			}
	...
	TEST(GenesisCompatibilityHarnessTests, StartupSequencingCoherenceCheckpointContextIsDeterministicAcrossRuns) {
		GenesisM68kBoundaryScaffold scaffold;
		vector<GenesisCompatibilityRomCase> corpus = BuildGenesisCompatibilityCorpus();

		auto readStartupSequencingCoherenceContext = [&](const GenesisCompatibilityMatrixResult& result, const string& romName) {
			auto entryIt = std::find_if(result.Entries.begin(), result.Entries.end(), [&](const GenesisCompatibilityEntry& entry) {
				return entry.Name == romName;
			});
			if (entryIt == result.Entries.end()) {
			...
			auto checkpointIt = std::find_if(entryIt->Checkpoints.begin(), entryIt->Checkpoints.end(), [](const GenesisCompatibilityCheckpoint& checkpoint) {
				return checkpoint.Id == "GEN-COMPAT-STARTUP-SEQUENCING-COHERENCE";
			});
			if (checkpointIt == entryIt->Checkpoints.end()) {
				ADD_FAILURE() << "Missing GEN-COMPAT-STARTUP-SEQUENCING-COHERENCE checkpoint for " << romName;
				return string();
			}
		...
		for (const GenesisCompatibilityRomCase& romCase : corpus) {
			string contextA = readStartupSequencingCoherenceContext(runA, romCase.Name);
			string contextB = readStartupSequencingCoherenceContext(runB, romCase.Name);
			EXPECT_EQ(contextA, contextB);
		}
	}

	TEST(GenesisCompatibilityHarnessTests, First20sSequencingStressCheckpointContextIsDeterministicAcrossRuns) {
		GenesisM68kBoundaryScaffold scaffold;
		vector<GenesisCompatibilityRomCase> corpus = BuildGenesisCompatibilityCorpus();

		auto readFirst20sSequencingStressContext = [&](const GenesisCompatibilityMatrixResult& result, const string& romName) {
			auto entryIt = std::find_if(result.Entries.begin(), result.Entries.end(), [&](const GenesisCompatibilityEntry& entry) {
				return entry.Name == romName;
			});
			if (entryIt == result.Entries.end()) {
			...
			auto checkpointIt = std::find_if(entryIt->Checkpoints.begin(), entryIt->Checkpoints.end(), [](const GenesisCompatibilityCheckpoint& checkpoint) {
				return checkpoint.Id == "GEN-COMPAT-FIRST20S-SEQUENCING-STRESS";
			});
			if (checkpointIt == entryIt->Checkpoints.end()) {
				ADD_FAILURE() << "Missing GEN-COMPAT-FIRST20S-SEQUENCING-STRESS checkpoint for " << romName;
				return string();
			}
		...
		for (const GenesisCompatibilityRomCase& romCase : corpus) {
			string contextA = readFirst20sSequencingStressContext(runA, romCase.Name);
			string contextB = readFirst20sSequencingStressContext(runB, romCase.Name);
			EXPECT_EQ(contextA, contextB);
		}
	}
