...
uint8_t PceVpc::Read(uint16_t addr) {
	if (_vdc2) {
		...
		switch (addr & 0x1F) {
			...
			case 0x08:
				return _state.Priority1;
			case 0x09:
				return _state.Priority2;

			case 0x0A:
				return (_state.Window1 & 0xFF);
			case 0x0B:
				return (_state.Window1 >> 8);
			case 0x0C:
				return (_state.Window2 & 0xFF);
			case 0x0D:
				return (_state.Window2 >> 8);
...
void PceVpc::Write(uint16_t addr, uint8_t value) {
	if (_vdc2) {
		...
		switch (addr & 0x1F) {
			...
			case 0x0A:
				_state.Window1 = (_state.Window1 & 0x300) | value;
				break;
			case 0x0B:
				_state.Window1 = (_state.Window1 & 0xFF) | ((value & 0x03) << 8);
				break;
			case 0x0C:
				_state.Window2 = (_state.Window2 & 0x300) | value;
				break;
			case 0x0D:
				_state.Window2 = (_state.Window2 & 0xFF) | ((value & 0x03) << 8);
				break;

			case 0x0E:
				_state.StToVdc2Mode = (value & 0x01) != 0;
				break;
...
void PceVpc::SetPriorityConfig(PceVpcPixelWindow wnd, uint8_t value) {
	...
	switch ((value >> 2) & 0x03) {
		case 0:
			cfg.PriorityMode = PceVpcPriorityMode::Default;
			break;
		case 1:
			cfg.PriorityMode = PceVpcPriorityMode::Vdc2SpritesAboveVdc1Bg;
			break;
		case 2:
			cfg.PriorityMode = PceVpcPriorityMode::Vdc1SpritesBelowVdc2Bg;
			break;
		case 3:
			cfg.PriorityMode = PceVpcPriorityMode::Default;
			break;
