...
void PceScsiBus::SetPhase(ScsiPhase phase) {
	...
	switch (_state.Phase) {
		...
		case ScsiPhase::MessageIn:
			SetSignals(Bsy, Cd, Io, Msg, Req);
			break;
		case ScsiPhase::Status:
			SetSignals(Bsy, Cd, Io, Req);
			break;
		case ScsiPhase::Busy:
			SetSignals(Bsy);
			break;
...
void PceScsiBus::CmdAudioEndPos() {
	...
	switch (_cmdBuffer[1]) {
		case 0:
			...
			break;
		case 1:
			player.SetEndPosition(endSector, CdPlayEndBehavior::Loop);
			break;
		case 2:
			player.SetEndPosition(endSector, CdPlayEndBehavior::Irq);
			break;
		case 3:
			player.SetEndPosition(endSector, CdPlayEndBehavior::Stop);
			break;
		case 4:
			break; // TODOPCE - the cd-rom might support this?
	}
...
void PceScsiBus::Exec() {
	...
	if (_updateCounter && --_updateCounter == 0) {
		switch (_updateType) {
			case ScsiUpdateType::SetCmdPhase:
				SetPhase(ScsiPhase::Command);
				break;
			case ScsiUpdateType::SetReqSignal:
				SetSignals(Req);
				break;
