...
uint8_t PceMemoryManager::ReadRegister(uint16_t addr) {
	if (addr <= 0x3FF) {
		...
		_state.IoBuffer = ((cdromConnected ? 0 : 0x80) |
		                   (isTurboGrafx ? 0 : 0x40) |
		                   0x30 |
		...
		switch (addr & 0x03) {
			...
			case 2:
				_state.IoBuffer = (_state.IoBuffer & 0xF8) | (_state.DisabledIrqs & 0x07);
				break;
			case 3:
				_state.IoBuffer = (_state.IoBuffer & 0xF8) | (_state.ActiveIrqs & 0x07);
				break;
		}
		return _state.IoBuffer;
	} else if (addr <= 0x1BFF) {
...
void PceMemoryManager::WriteRegister(uint16_t addr, uint8_t value) {
	if (addr <= 0x3FF) {
		...
		switch (addr & 0x03) {
			...
			case 2:
				_state.DisabledIrqs = value & 0x07;
				break;
			case 3:
				ClearIrqSource(PceIrqSource::TimerIrq);
				break;
		}
		_state.IoBuffer = value;
	} else if (addr <= 0x1BFF) {
...
AddressInfo PceMemoryManager::GetAbsoluteAddress(uint32_t relAddr) {
	...
	switch (_bankMemType[bank]) {
		case MemoryType::PcePrgRom:
			absAddr = (uint32_t)(_readBanks[bank] - _prgRom) + (relAddr & 0x1FFF);
			break;
		case MemoryType::PceWorkRam:
			absAddr = (uint32_t)(_readBanks[bank] - _workRam) + (relAddr & 0x1FFF);
			break;
		...
		case MemoryType::PceCdromRam:
			absAddr = (uint32_t)(_readBanks[bank] - _cdromRam) + (relAddr & 0x1FFF);
			break;
		case MemoryType::PceCardRam:
			absAddr = (uint32_t)(_readBanks[bank] - _cardRam) + (relAddr & 0x1FFF);
			break;
		default:
			return {-1, MemoryType::None};
	}
