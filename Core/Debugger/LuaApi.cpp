...
#define lua_pushintvalue(name, value) \
	lua_pushliteral(lua, #name);      \
	lua_pushinteger(lua, (int)value); \
	lua_settable(lua, -3);
#define lua_pushdoublevalue(name, value) \
	lua_pushliteral(lua, #name);         \
	lua_pushnumber(lua, (double)value);  \
	lua_settable(lua, -3);
#define lua_pushboolvalue(name, value) \
	lua_pushliteral(lua, #name);       \
	lua_pushboolean(lua, (int)value);  \
	lua_settable(lua, -3);
#define lua_pushstringvalue(name, value) \
	lua_pushliteral(lua, #name);         \
	lua_pushstring(lua, value.c_str());  \
	lua_settable(lua, -3);
#define lua_pusharrayvalue(index, value) \
	lua_pushinteger(lua, index);         \
	lua_pushinteger(lua, value);         \
	lua_settable(lua, -3);

#define lua_starttable(name)     \
	lua_pushliteral(lua, #name); \
	lua_newtable(lua);
#define lua_endtable() lua_settable(lua, -3);
#define lua_readint(name, dest)   \
	lua_getfield(lua, -1, #name); \
	dest = l.ReadInteger();
#define lua_readbool(name, dest)  \
	lua_getfield(lua, -1, #name); \
	dest = l.ReadBool();
#define error(text)        \
	luaL_error(lua, text); \
	return 0;
#define errorCond(cond, text)  \
	if (cond) {                \
		luaL_error(lua, text); \
		return 0;              \
	}
#define checkEnum(enumType, enumValue, text)               \
	if (!magic_enum::enum_contains<enumType>(enumValue)) { \
		luaL_error(lua, text);                             \
		return 0;                                          \
	}

#define checkparams()           \
	if (!l.CheckParamCount()) { \
		return 0;               \
	}
#define checkminparams(x)        \
	if (!l.CheckParamCount(x)) { \
		return 0;                \
	}
#define checkinitdone()                                             \
	if (!_context->CheckInitDone()) {                               \
		error("This function cannot be called outside a callback"); \
	}
#define checksavestateconditions()                                                                       \
	if (!_context->IsSaveStateAllowed()) {                                                               \
		error("This function must be called inside an exec memory operation callback for the main CPU"); \
	}
...
int LuaApi::GetAccessCounters(lua_State* lua) {
	...
	auto getValue = [&](AddressCounters& counter) -> uint64_t {
		switch (counterType) {
			default:
			case AccessCounterType::ReadCount:
				return counter.ReadCounter;
			case AccessCounterType::WriteCount:
				return counter.WriteCounter;
			case AccessCounterType::ExecCount:
				return counter.ExecCounter;
			case AccessCounterType::LastReadClock:
				return counter.ReadStamp;
			case AccessCounterType::LastWriteClock:
				return counter.WriteStamp;
			case AccessCounterType::LastExecClock:
				return counter.ExecStamp;
		}
	};
...
int LuaApi::GetState(lua_State* lua) {
	...
	for (auto& kvp : values) {
		...
		switch (kvp.second.Format) {
			case SerializeMapValueFormat::Integer:
				lua_pushinteger(lua, kvp.second.Value.Integer);
				break;
			case SerializeMapValueFormat::Double:
				lua_pushnumber(lua, kvp.second.Value.Double);
				break;
			case SerializeMapValueFormat::Bool:
				lua_pushboolean(lua, kvp.second.Value.Bool);
				break;
			case SerializeMapValueFormat::String:
				lua_pushlstring(lua, kvp.second.StringValue.c_str(), kvp.second.StringValue.size());
				break;
		}
		lua_settable(lua, -3);
	}
