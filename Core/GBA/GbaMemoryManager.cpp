...
uint8_t GbaMemoryManager::InternalRead(GbaAccessModeVal mode, uint32_t addr, uint32_t readAddr) {
	...
	switch (bank) {
		...
		case 0x02:
			return _extWorkRam[addr & (GbaConsole::ExtWorkRamSize - 1)];
		case 0x03:
			return _intWorkRam[addr & (GbaConsole::IntWorkRamSize - 1)];

		case 0x04:
			return ReadRegister(addr);

		case 0x05:
			return _palette[addr & (GbaConsole::PaletteRamSize - 1)];
...
void GbaMemoryManager::InternalWrite(GbaAccessModeVal mode, uint32_t addr, uint8_t value, uint32_t writeAddr, uint32_t fullValue) {
	...
	switch (bank) {
		...
		case 0x02:
			_extWorkRam[addr & (GbaConsole::ExtWorkRamSize - 1)] = value;
			break;
		case 0x03:
...
uint32_t GbaMemoryManager::ReadRegister(uint32_t addr) {
	if (addr < 0x60) {
		...
		switch (addr) {
			...
			case 0x200:
				return (uint8_t)_state.IE;
			case 0x201:
				return (uint8_t)(_state.IE >> 8);
			case 0x202:
				return (uint8_t)_state.IF;
			case 0x203:
				return (uint8_t)(_state.IF >> 8);
			case 0x204:
				return (uint8_t)_state.WaitControl;
			case 0x205:
				return (uint8_t)(_state.WaitControl >> 8);
			case 0x206:
				return 0;
			case 0x207:
				return 0;
			...
			case 0x300:
				return (uint8_t)_state.PostBootFlag;
			case 0x301:
				return 0;
			case 0x302:
				return 0;
...
AddressInfo GbaMemoryManager::GetAbsoluteAddress(uint32_t addr) {
	...
	switch (bank) {
		...
		case 0x02:
			return {(int32_t)addr & (GbaConsole::ExtWorkRamSize - 1), MemoryType::GbaExtWorkRam};

		case 0x03:
			return {(int32_t)addr & (GbaConsole::IntWorkRamSize - 1), MemoryType::GbaIntWorkRam};
		...
		case 0x05:
			return {(int32_t)addr & (GbaConsole::PaletteRamSize - 1), MemoryType::GbaPaletteRam};
		...
		case 0x07:
			return {(int32_t)addr & (GbaConsole::SpriteRamSize - 1), MemoryType::GbaSpriteRam};
...
int64_t GbaMemoryManager::GetRelativeAddress(AddressInfo& absAddress) {
	switch (absAddress.Type) {
		...
		case MemoryType::GbaExtWorkRam:
			return 0x2000000 | absAddress.Address;
		case MemoryType::GbaIntWorkRam:
			return 0x3000000 | absAddress.Address;
		case MemoryType::GbaPaletteRam:
			return 0x5000000 | absAddress.Address;
		case MemoryType::GbaPrgRom:
			return 0x8000000 | absAddress.Address;
		case MemoryType::GbaSaveRam:
			return _cart->GetRamRelativeAddress(absAddress);
		case MemoryType::GbaSpriteRam:
			return 0x7000000 | absAddress.Address;
		case MemoryType::GbaVideoRam:
			return 0x6000000 | absAddress.Address;
