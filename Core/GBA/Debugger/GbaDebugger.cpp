		...
		for (uint32_t i = 1; i < _dummyCpu->GetOperationCount(); i++) {
			...
			if (_breakpointManager->HasBreakpointForType(memOp.Type)) {
				...
				switch (_dummyCpu->GetOperationMode(i) & (GbaAccessMode::Byte | GbaAccessMode::HalfWord | GbaAccessMode::Word)) {
					case GbaAccessMode::Byte:
						_debugger->ProcessPredictiveBreakpoint<1>(CpuType::Gba, _breakpointManager.get(), memOp, absAddr);
						break;
					case GbaAccessMode::HalfWord:
						_debugger->ProcessPredictiveBreakpoint<2>(CpuType::Gba, _breakpointManager.get(), memOp, absAddr);
						break;
					case GbaAccessMode::Word:
						_debugger->ProcessPredictiveBreakpoint<4>(CpuType::Gba, _breakpointManager.get(), memOp, absAddr);
						break;
				}
			}
		}
...
void GbaDebugger::Step(int32_t stepCount, StepType type) {
	...
	switch (type) {
		...
		case StepType::PpuScanline:
			step.PpuStepCount = 308 * 4 * stepCount;
			break;
		case StepType::PpuFrame:
			step.PpuStepCount = 308 * 4 * 228 * stepCount;
			break;
...
uint8_t GbaDebugger::GetCpuFlags(uint32_t addr) {
	switch (_settings->GetDebugConfig().GbaDisMode) {
		case GbaDisassemblyMode::Default:
			return ((addr & 0x02) || _cpu->GetState().CPSR.Thumb) ? GbaCdlFlags::Thumb : 0;
		case GbaDisassemblyMode::Arm:
			return (addr & 0x02) ? GbaCdlFlags::Thumb : 0;
		case GbaDisassemblyMode::Thumb:
			return GbaCdlFlags::Thumb;
	}
	return 0;
}
