...
void BaseMapper::Initialize(NesConsole* console, RomData& romData) {
	...
	switch (romData.Info.BusConflicts) {
		...
		case BusConflictType::Yes:
			_hasBusConflicts = true;
			break;
		case BusConflictType::No:
			_hasBusConflicts = false;
			break;
	}
...
AddressInfo BaseMapper::GetRelativeAddress(AddressInfo& addr) {
	...
	switch (addr.Type) {
		...
		case MemoryType::NesMapperRam:
			ptrAddress = _mapperRam;
			break;
		case MemoryType::NesInternalRam:
			return {(int32_t)(addr.Address & _internalRamMask), MemoryType::NesMemory};
		default:
			return {GetPpuRelativeAddress(addr), MemoryType::NesPpuMemory};
	}
...
int32_t BaseMapper::GetPpuRelativeAddress(AddressInfo& addr) {
	...
	switch (addr.Type) {
		...
		case MemoryType::NesNametableRam:
			ptrAddress = _nametableRam;
			break;
		case MemoryType::NesMapperRam:
			ptrAddress = _mapperRam;
			break;
		case MemoryType::NesPaletteRam:
			return 0x3F00 | (addr.Address & 0x1F);
			break;
		default:
			return -1;
	}
