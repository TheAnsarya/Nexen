...
template <class T>
void NesPpu<T>::UpdateTimings(ConsoleRegion region, bool overclockAllowed) {
	_region = region;
...
template <class T>
void NesPpu<T>::UpdateVideoRamAddr() {
...
template <class T>
void NesPpu<T>::SetOpenBus(uint8_t mask, uint8_t value) {
...
template <class T>
uint8_t NesPpu<T>::ApplyOpenBus(uint8_t mask, uint8_t value) {
	SetOpenBus(~mask, value);
	return value | (_openBus & mask);
}

template <class T>
PpuModel NesPpu<T>::GetPpuModel() {
...
template <class T>
void NesPpu<T>::ProcessStatusRegOpenBus(uint8_t& openBusMask, uint8_t& returnValue) {
	switch (GetPpuModel()) {
...
template <class T>
uint8_t NesPpu<T>::PeekRam(uint16_t addr) {
...
template <class T>
uint8_t NesPpu<T>::ReadRam(uint16_t addr) {
	uint8_t openBusMask = 0xFF;
	uint8_t returnValue = 0;
	switch (GetRegisterID(addr)) {
		case PpuRegisters::Status:
...
template <class T>
void NesPpu<T>::WriteRam(uint16_t addr, uint8_t value) {
	if (addr != 0x4014) {
...
template <class T>
void NesPpu<T>::ProcessTmpAddrScrollGlitch(uint16_t normalAddr, uint16_t value, uint16_t mask) {
...
template <class T>
void NesPpu<T>::SetControlRegister(uint8_t value) {
	if (!_allowFullPpuAccess && _settings->GetNesConfig().RestrictPpuAccessOnFirstFrame) {
		return;
	}
...
template <class T>
void NesPpu<T>::SetMaskRegister(uint8_t value) {
	if (!_allowFullPpuAccess && _settings->GetNesConfig().RestrictPpuAccessOnFirstFrame) {
		return;
	}
...
template <class T>
void NesPpu<T>::UpdateStatusFlag() {
...
template <class T>
void NesPpu<T>::IncVerticalScrolling() {
...
// Taken from http://wiki.nesdev.com/w/index.php/The_skinny_on_NES_scrolling#Wrapping_around
template <class T>
void NesPpu<T>::IncHorizontalScrolling() {
...
template <class T>
uint16_t NesPpu<T>::GetNameTableAddr() {
	return 0x2000 | (_videoRamAddr & 0x0FFF);
}

// Taken from http://wiki.nesdev.com/w/index.php/The_skinny_on_NES_scrolling#Tile_and_attribute_fetching
template <class T>
uint16_t NesPpu<T>::GetAttributeAddr() {
...
template <class T>
uint8_t NesPpu<T>::ReadVram(uint16_t addr, MemoryOperationType type) {
	SetBusAddress(addr);
	return _mapper->ReadVram(addr, type);
}

template <class T>
void NesPpu<T>::WriteVram(uint16_t addr, uint8_t value) {
	SetBusAddress(addr);
...
template <class T>
void NesPpu<T>::LoadSprite(uint8_t spriteY, uint8_t tileIndex, uint8_t attributes, uint8_t spriteX, bool extraSprite) {
...
template <class T>
void NesPpu<T>::LoadExtraSprites() {
...
if (i == 0 && spriteBgColor != 0 && _sprite0Visible && _cycle != 256 && _mask.BackgroundEnabled && !_statusFlags.Sprite0Hit && _cycle > _minimumDrawSpriteStandardCycle) [[unlikely]] {
...
template <class T>
void NesPpu<T>::ProcessSpriteEvaluationStart() {
	_sprite0Added = false;
	_spriteInRange = false;
	_secondaryOamAddr = 0;
...
template <class T>
void NesPpu<T>::ProcessSpriteEvaluationEnd() {
...
template <class T>
void NesPpu<T>::ProcessSpriteEvaluation() {
...
template <class T>
uint8_t NesPpu<T>::ReadSpriteRam(uint8_t addr) {
	if (!_enableOamDecay) {
...
template <class T>
void NesPpu<T>::WriteSpriteRam(uint8_t addr, uint8_t value) {
...
template <class T>
uint16_t* NesPpu<T>::GetScreenBuffer(bool previousBuffer, bool processGrayscaleEmphasisBits) {
...
template <class T>
void NesPpu<T>::SendFrameVsDualSystem() {
...
template <class T>
void NesPpu<T>::BeginVBlank() {
	TriggerNmi();
}

template <class T>
void NesPpu<T>::TriggerNmi() {
	if (_control.NmiOnVerticalBlank) {
		_console->GetCpu()->SetNmiFlag();
	}
}
...
template <class T>
void NesPpu<T>::DebugUpdateFrameBuffer(bool toGrayscale) {
...
template <class T>
void NesPpu<T>::SetOamCorruptionFlags() {
...
template <class T>
void NesPpu<T>::ProcessOamCorruption() {
	if (!_console->GetNesConfig().EnablePpuOamRowCorruption) {
		return;
	}
...
template <class T>
void NesPpu<T>::ProcessScanlineFirstCycle() {
	_cycle = 0;
...
template <class T>
uint32_t NesPpu<T>::GetPixelBrightness(uint8_t x, uint8_t y) {
...
template <class T>
void NesPpu<T>::Serialize(Serializer& s) {
