...
void GbPpu::RunDrawCycle() {
	...
	if (!_wxEnableFlag) {
		...
		bool fetchWindow = (_state.WindowEnabled && //"Window enable bit in LCDC is set"
...
void GbPpu::ClockSpriteFetcher() {
	switch (_oamFetcher.Step++) {
		...
		case 3:
			_oamFetcher.LowByte = LcdReadVram(_oamFetcher.Addr);
			break;
...
void GbPpu::UpdateStatIrq() {
	bool irqFlag = (_state.LcdEnabled &&
...
uint8_t GbPpu::Read(uint16_t addr) {
	switch (addr) {
		...
		case 0xFF42:
			return _state.ScrollY; // FF42 - SCY - Scroll Y (R/W)
		case 0xFF43:
			return _state.ScrollX; // FF43 - SCX - Scroll X (R/W)
		case 0xFF44:
			return _state.Ly; // FF44 - LY - LCDC Y-Coordinate (R)
		case 0xFF45:
			return _state.LyCompare; // FF45 - LYC - LY Compare (R/W)
		case 0xFF47:
			return _state.BgPalette; // FF47 - BGP - BG Palette Data (R/W) - Non CGB Mode Only
		case 0xFF48:
			return _state.ObjPalette0; // FF48 - OBP0 - Object Palette 0 Data (R/W) - Non CGB Mode Only
		case 0xFF49:
			return _state.ObjPalette1; // FF49 - OBP1 - Object Palette 1 Data (R/W) - Non CGB Mode Only
		case 0xFF4A:
			return _state.WindowY; // FF4A - WY - Window Y Position (R/W)
		case 0xFF4B:
			return _state.WindowX; // FF4B - WX - Window X Position minus 7 (R/W)
	}
...
void GbPpu::Write(uint16_t addr, uint8_t value) {
	switch (addr) {
		...
		case 0xFF48:
			_state.ObjPalette0 = value;
			break;
		case 0xFF49:
			_state.ObjPalette1 = value;
			break;
		case 0xFF4A:
			_state.WindowY = value;
			break;
		case 0xFF4B:
			_state.WindowX = value;
			break;
...
uint8_t GbPpu::ReadCgbRegister(uint16_t addr) {
	...
	switch (addr) {
		case 0xFF4F:
			return _state.CgbVramBank | 0xFE;
		case 0xFF68:
			return _state.CgbBgPalPosition | (_state.CgbBgPalAutoInc ? 0x80 : 0) | 0x40;
		case 0xFF69:
			return ReadCgbPalette(_state.CgbBgPalPosition, _state.CgbBgPalettes);
		case 0xFF6A:
			return _state.CgbObjPalPosition | (_state.CgbObjPalAutoInc ? 0x80 : 0) | 0x40;
		case 0xFF6B:
			return ReadCgbPalette(_state.CgbObjPalPosition, _state.CgbObjPalettes);
	}
...
void GbPpu::WriteCgbRegister(uint16_t addr, uint8_t value) {
	...
	switch (addr) {
		case 0xFF4C:
			_state.CgbEnabled = (value & 0x0C) == 0;
			break;
		case 0xFF4F:
			_state.CgbVramBank = value & 0x01;
			break;
