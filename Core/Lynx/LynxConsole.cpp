...
LoadRomResult LynxConsole::LoadRom(VirtualFile& romFile) {
	...
	if (hasAtariLynxHeader && hasAtariLynxMeta) {
		...
		snprintf(cartInfo.Name, sizeof(cartInfo.Name), "%s", atariLynxMeta.CartName.c_str());
		snprintf(cartInfo.Manufacturer, sizeof(cartInfo.Manufacturer), "%s", atariLynxMeta.Manufacturer.c_str());
		...
		if (dbEntry) {
			...
			snprintf(cartInfo.Name, sizeof(cartInfo.Name), "%s", dbEntry->Name);
		}
	}
