...
NecDsp* NecDsp::InitCoprocessor(CoprocessorType type, SnesConsole* console, vector<uint8_t>& embeddedFirware) {
	...
	switch (type) {
		case CoprocessorType::DSP1:
			firmwareLoaded = FirmwareHelper::LoadDspFirmware(emu, FirmwareType::DSP1, "dsp1.rom", "dsp1.program.rom", "dsp1.data.rom", programRom, dataRom, embeddedFirware);
			break;
		case CoprocessorType::DSP1B:
			firmwareLoaded = FirmwareHelper::LoadDspFirmware(emu, FirmwareType::DSP1B, "dsp1b.rom", "dsp1b.program.rom", "dsp1b.data.rom", programRom, dataRom, embeddedFirware);
			break;
		case CoprocessorType::DSP2:
			firmwareLoaded = FirmwareHelper::LoadDspFirmware(emu, FirmwareType::DSP2, "dsp2.rom", "dsp2.program.rom", "dsp2.data.rom", programRom, dataRom, embeddedFirware);
			break;
		case CoprocessorType::DSP3:
			firmwareLoaded = FirmwareHelper::LoadDspFirmware(emu, FirmwareType::DSP3, "dsp3.rom", "dsp3.program.rom", "dsp3.data.rom", programRom, dataRom, embeddedFirware);
			break;
		case CoprocessorType::DSP4:
			firmwareLoaded = FirmwareHelper::LoadDspFirmware(emu, FirmwareType::DSP4, "dsp4.rom", "dsp4.program.rom", "dsp4.data.rom", programRom, dataRom, embeddedFirware);
			break;
		case CoprocessorType::ST010:
			firmwareLoaded = FirmwareHelper::LoadDspFirmware(emu, FirmwareType::ST010, "st010.rom", "st010.program.rom", "st010.data.rom", programRom, dataRom, embeddedFirware, 0xC000, 0x1000);
			break;
		case CoprocessorType::ST011:
			firmwareLoaded = FirmwareHelper::LoadDspFirmware(emu, FirmwareType::ST011, "st011.rom", "st011.program.rom", "st011.data.rom", programRom, dataRom, embeddedFirware, 0xC000, 0x1000);
			break;
		default:
			break;
	}
	...
	while (_state.CycleCount < targetCycle) {
		...
		switch (_opCode & 0xC00000) {
			...
			case 0x800000:
				...
				break;
			case 0xC00000:
				Load(_opCode & 0x0F, (uint16_t)(_opCode >> 6));
				break;
		}
...
void NecDsp::RunApuOp(uint8_t aluOperation, uint16_t source) {
	...
	switch (aluOperation) {
		...
		case 0x0A:
			...
			break;
		case 0x0B:
			result = (acc >> 1) | (acc & 0x8000);
			break;
		case 0x0C:
			result = (acc << 1) | (uint8_t)otherCarry;
			break;
		case 0x0D:
			result = (acc << 2) | 0x03;
			break;
...
void NecDsp::Jump() {
	...
	switch (jmpType) {
		case 0x00:
			_state.PC = _state.SerialOut;
			break;

		case 0x80:
			jmpCond = !_state.FlagsA.Carry;
			break;
		case 0x82:
			jmpCond = _state.FlagsA.Carry;
			break;
		case 0x84:
			jmpCond = !_state.FlagsB.Carry;
			break;
		case 0x86:
			jmpCond = _state.FlagsB.Carry;
			break;
		...
		case 0x88:
			jmpCond = !_state.FlagsA.Zero;
			break;
		case 0x8A:
			jmpCond = _state.FlagsA.Zero;
			break;
		case 0x8C:
			jmpCond = !_state.FlagsB.Zero;
			break;
		case 0x8E:
			jmpCond = _state.FlagsB.Zero;
			break;
		...
		case 0x90:
			jmpCond = !_state.FlagsA.Overflow0;
			break;
		case 0x92:
			jmpCond = _state.FlagsA.Overflow0;
			break;
		case 0x94:
			jmpCond = !_state.FlagsB.Overflow0;
			break;
		case 0x96:
			jmpCond = _state.FlagsB.Overflow0;
			break;
		case 0x98:
			jmpCond = !_state.FlagsA.Overflow1;
			break;
		case 0x9A:
			jmpCond = _state.FlagsA.Overflow1;
			break;
		case 0x9C:
			jmpCond = !_state.FlagsB.Overflow1;
			break;
		case 0x9E:
			jmpCond = _state.FlagsB.Overflow1;
			break;
		...
		case 0xA0:
			jmpCond = !_state.FlagsA.Sign0;
			break;
		case 0xA2:
			jmpCond = _state.FlagsA.Sign0;
			break;
		case 0xA4:
			jmpCond = !_state.FlagsB.Sign0;
			break;
		case 0xA6:
			jmpCond = _state.FlagsB.Sign0;
			break;
		case 0xA8:
			jmpCond = !_state.FlagsA.Sign1;
			break;
		case 0xAA:
			jmpCond = _state.FlagsA.Sign1;
			break;
		case 0xAC:
			jmpCond = !_state.FlagsB.Sign1;
			break;
		case 0xAE:
			jmpCond = _state.FlagsB.Sign1;
			break;
		...
		case 0xB0:
			jmpCond = !(_state.DP & 0x0F);
			break;
		case 0xB1:
			jmpCond = _state.DP & 0x0F;
			break;
		case 0xB2:
			jmpCond = (_state.DP & 0x0F) == 0x0F;
			break;
		case 0xB3:
			jmpCond = (_state.DP & 0x0F) != 0x0F;
			break;
		...
		case 0xB4:
			jmpCond = !(_state.SR & NecDspStatusFlags::SerialInControl);
			break;
		case 0xB6:
			jmpCond = _state.SR & NecDspStatusFlags::SerialInControl;
			break;
		case 0xB8:
			jmpCond = !(_state.SR & NecDspStatusFlags::SerialOutControl);
			break;
		case 0xBA:
			jmpCond = _state.SR & NecDspStatusFlags::SerialOutControl;
			break;
		case 0xBC:
			jmpCond = !(_state.SR & NecDspStatusFlags::RequestForMaster);
			break;
		case 0xBE:
			jmpCond = _state.SR & NecDspStatusFlags::RequestForMaster;
			break;
...
uint16_t NecDsp::GetSourceValue(uint8_t source) {
	switch (source) {
		...
		case 0x05:
			return _state.RP;
		case 0x06:
			return ReadRom(_state.RP);
		case 0x07:
			return 0x8000 - _state.FlagsA.Sign1;
