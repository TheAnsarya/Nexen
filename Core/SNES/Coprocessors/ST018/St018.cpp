...
AddressInfo St018::GetArmAbsoluteAddress(uint32_t addr) {
	switch (addr >> 28) {
		case 0x00:
			return {(int)(addr & 0x1FFFF), MemoryType::St018PrgRom};
		case 0x0A:
			return {(int)(addr & 0x7FFF), MemoryType::St018DataRom};
		case 0x0E:
			return {(int)(addr & 0x3FFF), MemoryType::St018WorkRam};
	}
...
int St018::GetArmRelativeAddress(AddressInfo& absoluteAddr) {
	switch (absoluteAddr.Type) {
		case MemoryType::St018PrgRom:
			return absoluteAddr.Address;
		case MemoryType::St018DataRom:
			return 0xA0000000 | absoluteAddr.Address;
		case MemoryType::St018WorkRam:
			return 0xE0000000 | absoluteAddr.Address;
	}
...
uint8_t St018::ReadCpuByte(uint32_t addr) {
	switch (addr >> 28) {
		...
		case 0x08:
			return 0;
		case 0x0A:
			return _dataRom[addr & 0x7FFF];
		case 0x0C:
			return 0;
		case 0x0E:
			return _workRam[addr & 0x3FFF];
	}
...
void St018::DebugWrite(uint32_t addr, uint8_t value) {
	switch (addr >> 28) {
		case 0x00:
			_prgRom[addr & 0x1FFFF] = value;
			break;
		case 0x0A:
			_dataRom[addr & 0x7FFF] = value;
			break;
		case 0x0E:
			_workRam[addr & 0x3FFF] = value;
			break;
	}
}
