...
uint8_t AluMulDiv::Peek(uint16_t addr) {
	switch (addr) {
		case 0x4214:
			return (uint8_t)_state.DivResult;
		case 0x4215:
			return (uint8_t)(_state.DivResult >> 8);

		case 0x4216:
			return (uint8_t)_state.MultOrRemainderResult;
		case 0x4217:
...
void AluMulDiv::Write(uint16_t addr, uint8_t value) {
	...
	switch (addr) {
		...
		case 0x4204:
			_state.Dividend = (_state.Dividend & 0xFF00) | value;
			break;
		case 0x4205:
			_state.Dividend = (_state.Dividend & 0xFF) | (value << 8);
			break;
