...
void Dsp::Write(uint8_t reg, uint8_t value) {
	...
	switch (reg & 0x0F) {
		case (int)DspVoiceRegs::Envelope:
			_state.EnvRegBuffer = value;
			break;
		case (int)DspVoiceRegs::Out:
			_state.OutRegBuffer = value;
			break;

		case 0x0C:
			switch (reg) {
				case (int)DspGlobalRegs::KeyOn:
					_state.NewKeyOn = value;
					break;
