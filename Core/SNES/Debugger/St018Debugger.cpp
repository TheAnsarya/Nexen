		...
		for (uint32_t i = 1; i < _dummyCpu->GetOperationCount(); i++) {
			...
			if (_breakpointManager->HasBreakpointForType(memOp.Type)) {
				...
				switch (_dummyCpu->GetOperationMode(i) & (ArmV3AccessMode::Byte | ArmV3AccessMode::Word)) {
					case ArmV3AccessMode::Byte:
						_debugger->ProcessPredictiveBreakpoint<1>(CpuType::St018, _breakpointManager.get(), memOp, absAddr);
						break;
					case ArmV3AccessMode::Word:
						_debugger->ProcessPredictiveBreakpoint<4>(CpuType::St018, _breakpointManager.get(), memOp, absAddr);
						break;
				}
			}
		}
