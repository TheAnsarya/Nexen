...
uint16_t SmsVdp::GetPixelColor() {
	...
	uint8_t color = (((_bgShifters[0] >> 23) & 0x01) |
...
void SmsVdp::WriteRegister(uint8_t reg, uint8_t value) {
	...
	switch (reg) {
		...
		case 3:
			_state.ColorTableAddress = value << 6;
			break;
		case 4:
			_state.BgPatternTableAddress = (value & 0x07) << 11;
			break;
		case 5:
			_state.SpriteTableAddress = (value & 0x7F) << 7;
			break;
		case 6:
			_state.SpritePatternSelector = (value & 0x07) << 11;
			break;
		case 7:
		...
		case 8:
			_state.HorizontalScroll = value;
			break;
		case 9:
			_state.VerticalScroll = value;
			break;
		case 10:
			_state.ScanlineCounter = value;
			break;
	}
}
...
uint8_t SmsVdp::ReadPort(uint8_t port) {
	switch (port & 0xC1) {
		case 0x40:
			return ReadVerticalCounter(); // V counter
		case 0x41:
			// H counter
			if (_latchRequest) {
		...
		case 0x81: {
			// Control Port
			uint8_t value = ((_state.VerticalBlankIrqPending ? 0x80 : 0) |
...
uint8_t SmsVdp::PeekPort(uint8_t port) {
	switch (port & 0xC1) {
		case 0x40:
			return ReadVerticalCounter(); // V counter
		case 0x41:
			return _state.HCounterLatch; // H counter
		case 0x80:
			return _state.VramBuffer;
