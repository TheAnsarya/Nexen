...
uint8_t SmsMemoryManager::InternalReadPort(uint8_t port) {
	...
	switch (_model) {
		case SmsModel::ColecoVision:
			value = ReadColecoVisionPort<isPeek>(port);
			break;
		case SmsModel::GameGear:
			value = ReadGameGearPort<isPeek>(port);
			break;
		default:
			value = ReadSmsPort<isPeek>(port);
	}
...
void SmsMemoryManager::WritePort(uint8_t port, uint8_t value) {
	...
	switch (_model) {
		case SmsModel::ColecoVision:
			WriteColecoVisionPort(port, value);
			break;
		case SmsModel::GameGear:
...
void SmsMemoryManager::WriteColecoVisionPort(uint8_t port, uint8_t value) {
	switch (port & 0xE0) {
		case 0x80:
			_controlManager->WriteControlPort(0);
			break;
		case 0xC0:
			_controlManager->WriteControlPort(1);
			break;
		case 0xE0:
			_psg->Write(value);
			break;
...
void SmsMemoryManager::WriteGameGearPort(uint8_t port, uint8_t value) {
	switch (port) {
		...
		case 1:
			_state.GgExtData = value & 0x7F;
			break;
		case 2:
			_state.GgExtConfig = value;
			break;
		case 3:
			_state.GgSendData = value;
			break;
		case 4:
			break; // read-only
		case 5:
			_state.GgSerialConfig = value;
			break;
...
uint8_t SmsMemoryManager::ReadSmsPort(uint8_t port) {
	switch (port & 0xC1) {
		case 0x00:
			return GetOpenBus(); // Port 3E (Memory Control)
		...
		case 0xC1:
			return _state.IoEnabled ? _controlManager->ReadPort(1) : GetOpenBus(); // Port DD (IO Port B/Misc)
...
uint8_t SmsMemoryManager::ReadColecoVisionPort(uint8_t port) {
	switch (port & 0xE0) {
		case 0xA0:
			return isPeek ? _vdp->PeekPort(port) : _vdp->ReadPort(port);
		case 0xE0:
			return _controlManager->ReadPort((port >> 1) & 0x01);
		default:
			return 0xFF;
	}
}
...
uint8_t SmsMemoryManager::ReadGameGearPort(uint8_t port) {
	switch (port) {
		...
		// TODOSMS GG - input/output ext port
		case 1:
			return _state.GgExtData;
		case 2:
			return _state.GgExtConfig;
		case 3:
			return _state.GgSendData;
		case 4:
			return 0xFF;
		case 5:
			return _state.GgSerialConfig;
