...
public partial class StateGridEntry : UserControl {
	...
	private static readonly IBrush BadgePurple = new SolidColorBrush(Color.Parse("#9b59b6"));  // Designated slot (current)
	private static readonly IBrush BadgePurpleDark = new SolidColorBrush(Color.Parse("#6b5b7b"));  // Designated slot (historical) — muted gray-purple, clearly "faded"
	...
	public void Init() {
		...
		if (game.IsTimestampedSave) {
			...
			(BadgeText, BadgeBackground) = game.Origin switch {
				...
				SaveStateOrigin.Designated => (
					game.SlotNumber > 0 ? $"Slot {game.SlotNumber:d2}" : "Slot",
					game.IsCurrentSlot ? BadgePurple : BadgePurpleDark
				),
