...
public partial class SpritePreviewPanel : NexenUserControl {
	...
	public SpritePreviewPanel(SpritePreviewModel model, SpriteViewerConfig config) {
		...
		AddDisposable(this.WhenAnyValue(x => x.Model.FadePreview, x => x.Config.DimOffscreenSprites).Subscribe(x => FadePreview = Model.FadePreview == true && Config.DimOffscreenSprites == true));
	}
