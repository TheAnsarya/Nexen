...
public partial class HexEditor : Control {
	...
	static HexEditor() {
		...
		FontFamilyProperty.Changed.AddClassHandler<HexEditor>((x, e) => x.InitFontAndLetterSize());
	...
	private void ChangeSelectionLength(int offset) {
		...
		this._cursorPosition = newAnchorEnd ? this.SelectionStart + this.SelectionLength : this.SelectionStart;
...
#pragma warning restore CS0618
	...
	protected override void OnPointerPressed(PointerPressedEventArgs e) {
		...
		if (props.IsLeftButtonPressed) {
			...
			LastNibble = e.KeyModifiers.HasFlag(KeyModifiers.Control) && gridPos.Value.LastNibble;
...
public struct ByteInfo {
	...
	public override bool Equals(object? obj) {
		if (obj is null or not ByteInfo) {
			return false;
		}
