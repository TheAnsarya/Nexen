...
public partial class PaletteSelector : Control {
	...
	public PaletteSelector() {
		this.GetObservable(SelectionModeProperty).Subscribe((mode) => this.CoerceValue(SelectedPaletteProperty));

		this.GetObservable(PaletteColorsProperty).Subscribe((mode) => this.CoerceValue(SelectedPaletteProperty));
		...
		for (int y = 0, max = paletteColors.Length / columnCount; y < max; y++) {
			for (int x = 0; x < columnCount; x++) {
				...
				if (ShowIndexes) {
					...
					string label = index < PaletteIndexValues?.Length ? PaletteIndexValues[index].ToString("X2") : index.ToString("X2");
