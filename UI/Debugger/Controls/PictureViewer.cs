...
public sealed class PictureViewer : Control {
	...
	static PictureViewer() {
		...
		ZoomProperty.Changed.AddClassHandler<PictureViewer>((x, e) => x.UpdateSize());
	...
	public void ZoomIn() {
		Zoom = Zoom < 1 ? 1 : Math.Min(40, Math.Max(1, Zoom + 1));
	}

	public void ZoomOut() {
		Zoom = Zoom <= 1 ? 0.5 : Math.Min(40, Math.Max(1, Zoom - 1));
	}
